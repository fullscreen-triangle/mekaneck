...
\textbf{Drug Coupling}: Photosensitizers and photodynamic therapy agents (porphyrins, phthalocyanines) couple at this level through singlet-triplet intersystem crossing, generating reactive oxygen species via $^1$O$_2$ (singlet oxygen).
...
\textbf{Physical Mechanism}: Collective vibrational modes of protein backbone and side chains. Normal mode frequencies:

\begin{equation}
\omega_k = \sqrt{\frac{\lambda_k}{m_{\text{eff}}}}
\end{equation}

where $\lambda_k$ are eigenvalues of Hessian matrix and $m_{\text{eff}}$ is effective mass.
...
COX-2 active site has dominant mode at $\omega_{\text{COX}} = 8.5 \times 10^{11}$ rad/s (45 cm$^{-1}$). Aspirin acetyl group stretch at $\omega_{\text{aspirin}} = 8.0 \times 10^{11}$ rad/s matches within 6\%, enabling resonant coupling and irreversible acetylation.
...
where $m$ is activation gate variable and $\alpha_m, \beta_m$ are voltage-dependent rate constants.

\textbf{Oscillation}: Gate transitions occur at frequency:

\begin{equation}
\omega_{\text{gate}} = \alpha_m + \beta_m \sim 10^9 \text{ s}^{-1} = 1 \text{ GHz}
\end{equation}

\textbf{Drug Coupling}: Channel blockers (lidocaine, tetrodotoxin) stabilize closed state by increasing energy barrier:
...
Lidocaine $K_d = 100$ μM corresponds to $\Delta G_{\text{binding}} = -5.5$ kcal/mol. This modulates activation rate:
...
\textbf{BMD Operation}: Substrate binding (measurement) → transition state stabilization (feedback) → product release (reset)
...
Typical drug couples at Level 2 (protein conformational, $10^{12}$ Hz), propagates:
...
This corresponds to daily timescale (Level 8), explaining therapeutic effects manifest over days-weeks despite molecular binding occurring in picoseconds.
...
\textbf{PK}: Absorption, distribution, metabolism, excretion determine $[D](t)$ at target site.
...
Explains clinical observation: symptomatic relief (Level 5-6) in hours-days, disease modification (Level 7-8) in weeks-months.

\subsection{Thermodynamic Cost Across Hierarchy}

Total BMD operation cost summed across levels:
