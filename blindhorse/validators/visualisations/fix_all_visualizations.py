#!/usr/bin/env python3
"""
Fix all visualization scripts to work with actual JSON data structure.
"""
...
def fix_state_py():
    """Fix state.py to match actual data structure"""
    content = """import json
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
if __name__ == "__main__":
    import sys
    json_file = sys.argv[1] if len(sys.argv) > 1 else '../results/categorical_state/categorical_state_results.json'
    output_file = sys.argv[2] if len(sys.argv) > 2 else 'categorical_state_figure.png'
    create_categorical_state_figure(json_file, output_file)
"""
    
    with open('blindhorse/validators/visualisations/state.py', 'w') as f:
        f.write(content)
    print("✓ Fixed state.py")

def main():
    print("Fixing visualization scripts...")
    fix_state_py()
    print("\n✓ All visualization scripts fixed!")
