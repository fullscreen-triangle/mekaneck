...
\input{figures/primitive-captions}
...
\emph{The two failure rates coincide exactly.} Sweeping the probability that
catalysts agree, over $400$ substrates at each of $20$ agreement levels, the
fraction of runs on which closure declines and the fraction on which the
threshold rule reports a single answer \emph{despite} split evidence are
equal at every level---$0.268$, $0.765$ and $0.280$ at agreement
probabilities $0.1$, $0.5$ and $0.9$ respectively. The coincidence is not a
coefficient to be estimated: whenever the reachable set contains more than
one cell, some catalyst reaches the cell that satisfies the threshold, and
the threshold rule halts there by construction. Hence on this family of
substrates the threshold rule fails on \emph{exactly} the runs where the
evidence is contested, with no residue of cases in which it happens to be
right. The rate is non-monotone in agreement, peaking near $0.5$, because
both very high and very low agreement produce unanimity---at one extreme on
the confident cell, at the other on its alternative.
...
\Cref{thm:closure-stronger} realised in a concrete evaluation: internal
